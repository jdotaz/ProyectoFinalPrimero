...
Animal::Animal(int codigo_,string nombre_,int especie_,string raza_,string color_,int day,int month, int year,int sexo_,bool existente=1){
  ...
  sexo=sexo_;
  if(!existente){
    string archivo=to_string(codPropietario)+".txt";
    fstream crear(archivo);
    crear.seekg(0,ios::end);
    crear<<codPropietario<<endl;
    crear<<nombre<<endl;
    crear<<especie<<endl;
    crear<<raza<<endl;
    crear<<color<<endl;
    crear<<nacimiento[0]<<endl;
    crear<<nacimiento[1]<<endl;
    crear<<nacimiento[2]<<endl;
    crear<<sexo<<endl;
  }
}
